...
const KitchenPage = () => {
  ...
  useEffect(() => {
    RestServices.kitchenOrdersController.fetchUncompletedKitchenOrders().then((res) => setKitchenOrders(res));
  }, []);

  const handleCompleteOrder = async (kitchenOrderUuid: string) => {
    await RestServices.kitchenOrdersController.markKitchenOrderAsCompleted(kitchenOrderUuid).then((res) => {
      toast.success(res);
    });

    await RestServices.kitchenOrdersController.fetchUncompletedKitchenOrders().then((res) => setKitchenOrders(res));
  };
  ...
  const columns: ColumnsType<KitchenOrder> = [
    ...
    },
    {
      ...
      render: (_, item) => <span>{item.waiter?.firstName + " " + item.waiter?.lastName}</span>,
    },
    {
      title: intl.formatMessage({
          ...
          <>
            <div style={{ display: "flex", gap: "1rem" }}>
              ...
              <p>{`${intl.formatMessage({
                ...
                currentLanguage === "en" ? kitchenOrder.product?.name : kitchenOrder.product?.nameTranslated
              },`}</p>
              <p>{`${intl.formatMessage({
                id: "kitchenApplication.table.column.listOfProducts.quantity",
              })}: ${kitchenOrder.quantity}${kitchenOrder.product?.listOfIngredients.length !== 0 ? "," : ""}`}</p>
              {kitchenOrder.product!.listOfIngredients.length === 0 ? null : (
                <p>{`${intl.formatMessage({
                  id: "kitchenApplication.table.column.listOfProducts.ingredients",
                })}:  ${kitchenOrder.product?.listOfIngredients.map((ingredient, index) => {
                  const ingName = currentLanguage === "en" ? ingredient.name : ingredient.nameTranslated;
                  if (index === (kitchenOrder.product?.listOfIngredients.length as number) - 1) {
                    return ingName;
                  } else {
                    return ingName + ", ";
                  }
    ...
    },
    {
      ...
      render: (_, kitchenOrder) => (
        <Popconfirm
          ...
          <Button type="primary">{intl.formatMessage({ id: "kitchenApplication.button.text" })}</Button>
        </Popconfirm>
      ),
    },
  ];
  ...
  return (
    <Content style={{ backgroundColor: "#4d4b4b" }}>
      <Table<KitchenOrder> rowKey={(order) => order.uuid as string} columns={columns} dataSource={kitchenOrders} />
    </Content>
  );
};
